"""`PREDICTIVE-STAGE2-OPEN-INTEREST-V1` — leveraged positioning quantity.

Two preregistered configurations, `OPEN_INTEREST_LINEAR_DUAL_HEAD_V1` and
`OPEN_INTEREST_HGBR_DUAL_HEAD_V1`, both executed in this work package regardless of the first
result, on six causal features derived only from as-of hourly open-interest states.

The frozen target, labels, folds, scorer, reliability bins and dependence-aware bootstrap are
those of `PREDICTIVE_EVALUATION_CONTRACT_V1` Amendment A1 and `PREDICTIVE-BASELINES-V1`. The
evaluation fold set is not chosen here: it is whatever the pre-result source audit declared
source-admissible, which for this source is decided by archive history and source quality
alone.

The label, fold, scoring and paired-inference modules are imported unchanged; their bytes are
hashed by four executed admissions and may not move.
"""
...
from .open_interest_audit import (
    AUDIT_PATH,
...
LINEAR_MODEL_VERSION = "OPEN_INTEREST_LINEAR_DUAL_HEAD_V1"
HGBR_MODEL_VERSION = "OPEN_INTEREST_HGBR_DUAL_HEAD_V1"
CONFIGURATION_ORDER = (LINEAR_MODEL_VERSION, HGBR_MODEL_VERSION)

EXPERIMENT_IDS = {
    LINEAR_MODEL_VERSION: "EXP-PRED-005-OPEN-INTEREST-LINEAR-DUAL-HEAD",
    HGBR_MODEL_VERSION: "EXP-PRED-006-OPEN-INTEREST-HGBR-DUAL-HEAD",
}
HYPOTHESIS_IDS = {
    LINEAR_MODEL_VERSION: "H-PRED-OI-001",
    HGBR_MODEL_VERSION: "H-PRED-OI-002",
}

SEARCH_PLAN_PATH = "research/protocols/PREDICTIVE-STAGE2-OPEN-INTEREST-SEARCH-PLAN-V1.json"
ADMISSION_PATH = "reports/validation/PREDICTIVE-STAGE2-OPEN-INTEREST-V1-ADMISSION.json"
REPORT_JSON_PATH = "reports/research/PREDICTIVE-STAGE2-OPEN-INTEREST-V1.json"
REPORT_MARKDOWN_PATH = "reports/research/PREDICTIVE-STAGE2-OPEN-INTEREST-V1.md"
...
GATE_NAMES = (
    ...
    "POOLED_DELTA_VERSUS_MATCHED_ALWAYS_UP_AT_LEAST_MESI",
    "PAIRED_INTERVAL_LOWER_BOUND_ABOVE_ZERO",
    "WIN_RATE_AT_LEAST_MATCHED_TRAINING_BASE_RATE",
    "BRIER_AT_MOST_MATCHED_TRAINING_BASE_RATE_BRIER",
    "AT_LEAST_TWO_THIRDS_OF_FOLDS_NON_NEGATIVE",
)

ADVANCE = {
    LINEAR_MODEL_VERSION: "ADVANCE_OPEN_INTEREST_LINEAR_V1",
    HGBR_MODEL_VERSION: "ADVANCE_OPEN_INTEREST_HGBR_V1",
}
NO_ADVANCE = {
    LINEAR_MODEL_VERSION: "NO_ADVANCE_OPEN_INTEREST_LINEAR_V1",
    HGBR_MODEL_VERSION: "NO_ADVANCE_OPEN_INTEREST_HGBR_V1",
}
...
IMPLEMENTATION_FILES = (
    ...
    "backend/app/predictive/open_interest.py",
    "backend/app/predictive/open_interest_audit.py",
    "backend/app/predictive/open_interest_source.py",
    "backend/app/predictive/paired_inference.py",
    "scripts/run_predictive_open_interest.py",
)
...
def director_decisions() -> dict[str, Any]:
    """The Research Director's post-funding decisions, recorded before execution."""
    return {
        "decision_id": "PREDICTIVE_STAGE2_OPEN_INTEREST_ADMISSION_V1",
        "decided_by": "RESEARCH_DIRECTOR",
        "generation_continues": True,
        "generation": RESEARCH_GENERATION,
        "target_or_horizon_changed": False,
        "target_change_disposition": "DEFERRED_WOULD_CREATE_A_NEW_RESEARCH_GENERATION",
        ...
        "admitted_source": "BTCUSDT_USDM_PERPETUAL_OPEN_INTEREST",
        "admission_rationale": (
            "Open interest measures the quantity of leveraged exposure, whereas settled "
            "funding already tested a price/carry channel, so the two families are "
            "orthogonal."
        ),
...
def search_plan() -> dict[str, Any]:
    """The Stage-2 open-interest model family, frozen before any candidate result."""
    return {
        "version": "PREDICTIVE_STAGE2_OPEN_INTEREST_SEARCH_PLAN_V1",
        ...
        "purpose": (
            "Freeze the complete Stage-2 open-interest model family before either "
        ...
        "forbidden": [
            "THIRD_STAGE2_OPEN_INTEREST_MODEL",
            "HYPERPARAMETER_SEARCH",
            "THRESHOLD_SEARCH",
            "FEATURE_SEARCH",
            "RESULT_DEPENDENT_EARLY_STOP",
            "POST_HOC_WINNER_SELECTION",
            "COMBINING_OPEN_INTEREST_WITH_REJECTED_STAGE1_OR_FUNDING_FEATURES",
            "BASIS_LONG_SHORT_RATIO_CFTC_MACRO_NEWS_ONCHAIN_ADMISSION",
            "STAGE1_SUBSTRATE_REPAIR",
            "ASSET_UNIVERSE_EXPANSION",
...
def specification(model_version: str) -> dict[str, Any]:
    ...
    if model_version not in EXPERIMENT_IDS:
        raise ExperimentError(f"unknown Stage-2 open-interest configuration: {model_version}")
...
def preregistration(model_version: str, root: Path = ROOT) -> dict[str, Any]:
    ...
    return {
        ...
        "hypothesis_id": HYPOTHESIS_IDS[model_version],
        "hypothesis": (
            "The quantity of leveraged BTCUSDT perpetual positioning, measured by official "
            "historical open interest and read strictly point-in-time, contains "
            "out-of-sample information about the frozen BTCUSDT spot 24h terminal direction "
            "beyond the matched ALWAYS_UP baseline."
        ),
        ...
        "features": {
            ...
            "stage1_or_funding_features_combined": False,
            "price_derived_feature_present": False,
        },
        ...
        "controls": {
            ...
            "calibration_and_directional_reference": CALIBRATION_CONTROL,
            "calibration_control_semantics": "AMENDMENT_A1_TRAINING_ONLY_REFIT_PER_FOLD",
            "descriptive": [PREVIOUS_24H_SIGN_PERSISTENCE],
            "magnitude": ZERO_RETURN_MAGNITUDE,
        ...
        "primary_effect": {
            "definition": (
                "candidate actionable directional win rate - matched ALWAYS_UP win rate on "
                "the exact candidate-actionable timestamps"
            ),
        ...
        "advancement_gate": {
            ...
            "conditions": [
                ...
                {"name": GATE_NAMES[4], "threshold": "MATCHED_TRAINING_BASE_RATE_WIN_RATE"},
...
def admission(root: Path = ROOT) -> dict[str, Any]:
    ...
    if not (root / CONTRACT_PATH).is_file():
        raise ExperimentError("the predictive open-interest contract must exist before execution")
...
def _feature_cache(
    source: OpenInterestSource, labels: Sequence[Label]
...
def _fit_magnitude(
    ...
    linear = model_version == LINEAR_MODEL_VERSION
    scaler = StandardScaler().fit(matrix) if linear else None
    design = matrix if scaler is None else scaler.transform(matrix)
    regressor = (
        Ridge(**LINEAR_MAGNITUDE_PARAMETERS)
        if linear
        else HistGradientBoostingRegressor(loss=HGBR_MAGNITUDE_LOSS, **HGBR_PARAMETERS)
    )
...
def _paired_records(
    predictions: Sequence[Prediction], matched_labels: Sequence[Label]
) -> list[PairedRecord]:
    ...
    return [
        PairedRecord(
            ...
            baseline_correct=truth[moment].direction == UP,
        )
...
def advancement_gate(
    ...
    conditions: dict[str, dict[str, Any]] = {
        ...
        GATE_NAMES[4]: {
            "threshold": base_rate_win_rate,
            "observed": candidate_win_rate,
            "passed": candidate_win_rate >= base_rate_win_rate,
        },
        GATE_NAMES[5]: {
...
def baseline_context(root: Path = ROOT) -> dict[str, Any]:
    ...
    return {
        ...
        "note": "SIX_FOLD_FULL_UNIVERSE_NOT_THE_OPEN_INTEREST_ADMISSIBLE_UNIVERSE",
...
def _configuration_result(
    ...
    for fold in folds:
        ...
        paired_folds.append(
            build_paired_fold(
                ...
                _paired_records(directional, matched_labels),
            )
        )
        ...
        fold_deltas[fold.name] = float(candidate_rate) - float(controls[ALWAYS_UP]["win_rate"])
        by_fold[fold.name] = {
            "fit": fit_record,
            ...
            "controls": controls,
            "delta_versus_always_up": fold_deltas[fold.name],
        }
    ...
    pooled_controls = {
        ...
        ZERO_RETURN_MAGNITUDE: score(
            zero_return_magnitude_predictions(pooled_matched),
            pooled_outcomes,
            declares_direction=False,
            declares_probability=False,
            declares_magnitude=True,
    ...
    base_rate_rate = float(pooled_controls[CALIBRATION_CONTROL]["win_rate"])
    candidate_brier = float(pooled_directional_score["brier_score"])
    base_rate_brier = float(pooled_controls[CALIBRATION_CONTROL]["brier_score"])
    ...
    return {
        ...
        "primary_comparison": {
            "definition": (
                "candidate actionable directional win rate - matched ALWAYS_UP win rate on "
                "candidate-actionable, non-NEUTRAL timestamps"
            ),
            "matched_control": ALWAYS_UP,
        ...
        "base_rate_comparison": {
            ...
            "matched_win_rate": base_rate_rate,
            "candidate_at_least_control": candidate_rate >= base_rate_rate,
            ...
            "note": "BEATING_THIS_CONTROL_ALONE_IS_NOT_USEFULNESS",
        },

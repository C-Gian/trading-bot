"""Frozen fitting procedures for the deterministic-calendar Generation V2 family."""
...
from . import DOWN, UP
from .calendar_features import FEATURE_COUNT
from .labels import HOUR_SECONDS

LINEAR_MODEL_VERSION = "V2_CALENDAR_LINEAR_V1"
HGBR_MODEL_VERSION = "V2_CALENDAR_HGBR_V1"
SKLEARN_VERSION = "1.7.2"
INFERENCE_SEED = 20260921
...
class CalendarModelError(RuntimeError):
...
class CalendarProbabilityHead:
    ...
    def probability_up(self, features: Sequence[Sequence[float]]) -> list[float]:
        ...
        if matrix.ndim != 2 or matrix.shape[1] != FEATURE_COUNT:
            raise CalendarModelError("the calendar head was given the wrong feature width")
        ...
        if not np.all(np.isfinite(probabilities)) or not np.all(
            ...
            raise CalendarModelError("the calibrated probability output is invalid")
...
def calibration_split(open_times: Sequence[int]) -> CalibrationSplit:
    ...
    if total < 2:
        raise CalendarModelError("the outer-training portion is too small to split")
    ordered = list(open_times)
    if any(ordered[index] <= ordered[index - 1] for index in range(1, total)):
        raise CalendarModelError("the outer-training portion is not strictly chronological")
    ...
    if base_fit == 0 or calibration == 0:
        raise CalendarModelError("the frozen calibration split leaves an unusable side")
...
def fit_probability_head(
    ...
    if model_version not in {LINEAR_MODEL_VERSION, HGBR_MODEL_VERSION}:
        raise CalendarModelError(f"unknown frozen calendar model: {model_version}")
    if not (len(open_times) == len(features) == len(directions)):
        raise CalendarModelError("the training rows are not aligned")
    if any(direction not in {UP, DOWN} for direction in directions):
        raise CalendarModelError("a training row carries a non-directional label")

    split = calibration_split(open_times)
    matrix = np.asarray(features, dtype=np.float64)
    if matrix.ndim != 2 or matrix.shape[1] != FEATURE_COUNT:
        raise CalendarModelError("the training matrix has the wrong feature width")
    if not np.all(np.isfinite(matrix)):
        raise CalendarModelError("the training matrix contains an invalid calendar feature")
    ...
    if len(np.unique(truth[base_mask])) < 2 or len(np.unique(truth[calibration_mask])) < 2:
        raise CalendarModelError("a frozen fitting side lacks both directional classes")
    ...
    if list(platt.classes_) != [0, 1]:
        raise CalendarModelError("the Platt map did not see both directional classes")
    return CalendarProbabilityHead(

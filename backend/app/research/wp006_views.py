...
def wp006_accounting_snapshot(root: Path = ROOT) -> dict[str, int]:
    """Reconstruct the fixed cumulative counters at the WP-006 checkpoint.

    Later append-only registry allocations must not make validation of the immutable
    WP-006 checkpoint depend on current work-package state.
    """

"""Execute or validate the frozen V2 internal-structure selective family."""
...
def execute() -> dict:
    ...
    if not path.is_file():
        raise InternalSelectiveExperimentError("the pre-result admission artifact does not exist")
...
def main() -> None:
    ...
    if options.check:
        finding = validate_internal_selective(ROOT, data_available=options.data)
        print(
            "predictive V2 internal structure: "
    ...
    report = execute()
    print(
        "predictive V2 internal structure written: "

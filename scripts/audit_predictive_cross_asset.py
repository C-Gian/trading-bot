"""Run the pre-result cross-asset source audit and write its immutable record.

Provenance, point-in-time semantics, the minimum point-in-time universe and fold coverage are
decided here, before any target-bearing model exists. ``--check`` revalidates the committed
audit instead of recomputing it.
"""
...
from app.predictive.cross_asset_audit import AUDIT_PATH, PASS, run_source_audit
...
def main() -> None:
    ...
    if options.check:
        ...
        print(
            f"cross asset source audit: {committed['status']} "
    ...
    print(f"cross asset source audit: {audit['status']}")
    if coverage is not None:
        for name, record in coverage["by_fold"].items():
            print(
                ...
                f"history_days={record['causal_pre_fold_history_days']:.0f} "

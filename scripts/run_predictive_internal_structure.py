"""Execute the frozen `PREDICTIVE-INTERNAL-STRUCTURE-V1` experiment exactly once.

The pre-execution admission artifact must already be committed and must still match the
frozen records and the implementation, so the preregistration provably precedes the first
outer-evaluation number. ``--check`` validates the committed artifacts instead of
recomputing them; ``--data`` additionally replays them against the installed market data.
"""
...
from app.predictive.internal_report import (
    markdown_bytes,
    report_bytes,
...
def main() -> None:
    ...
    if options.check:
        findings = validate_internal_structure(ROOT, data_available=options.data)
        print(
            f"predictive internal structure: {findings['status']} "
    ...
    print(
        f"predictive internal structure written: "

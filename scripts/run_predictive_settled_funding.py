"""Execute both frozen `PREDICTIVE-STAGE2-SETTLED-FUNDING-V1` configurations exactly once.

The pre-execution admission artifact must already be committed and must still match the
frozen search plan, both preregistrations, the predictive funding contract, the canonical
source identity and the implementation. ``--check`` validates the committed artifacts instead
of recomputing them; ``--data`` additionally replays them against the installed data.
"""
...
from app.predictive.settled_funding_report import (
...
def main() -> None:
    ...
    if options.check:
        findings = validate_settled_funding(ROOT, data_available=options.data)
        print(
            f"predictive stage 2 settled funding: {findings['status']} "
    ...
    result = execute()
    print(
        f"predictive stage 2 settled funding written: {result['family_disposition']['disposition']}"
    )
    for model_version in CONFIGURATION_ORDER:

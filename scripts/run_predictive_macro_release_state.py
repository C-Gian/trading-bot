"""Execute both frozen `PREDICTIVE-STAGE3-MACRO-RELEASE-STATE-V1` configurations exactly once.

The pre-execution admission artifact must already be committed and must still match the frozen
search plan, both preregistrations, the release-state contract, the passed source audit, the
governed source identity, the predecessor's pinned records and the implementation. ``--check``
validates the committed artifacts instead of recomputing them; ``--data`` additionally replays
them.
"""
...
def main() -> None:
    ...
    if options.check:
        findings = validate_macro_release_state(ROOT, data_available=options.data)
        print(
            f"predictive stage 3 macro release state: {findings['status']} "
    ...
    result = execute()
    print(
        "predictive stage 3 macro release state written: "

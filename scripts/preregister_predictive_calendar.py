"""Freeze the V2 calendar search family before any outer prediction is produced."""
...
def main() -> None:
    if any((ROOT / RESULT_PATHS[model]).exists() for model in CONFIGURATION_ORDER):
        raise CalendarExperimentError("refusing to rewrite preregistration after a result exists")

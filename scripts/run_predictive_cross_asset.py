"""Execute both frozen `PREDICTIVE-STAGE3-CROSS-ASSET-BREADTH-V1` configurations exactly once.
...
the governed source identity and the implementation. ``--check`` validates the committed
...
from app.predictive.cross_asset_report import (
...
def main() -> None:
    ...
    if options.check:
        findings = validate_cross_asset(ROOT, data_available=options.data)
        print(
            f"predictive stage 3 cross asset breadth: {findings['status']} "
    ...
    result = execute()
    print(
        "predictive stage 3 cross asset breadth written: "

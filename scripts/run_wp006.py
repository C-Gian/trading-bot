"""Execute only the committed WP-006 declaration; no optimization or data acquisition.

Every gate — allocation, novelty admission, registry, ledger, preregistration
chronology — runs before a single market array is loaded.
"""
...
from app.research.pullback_lab import PullbackLab
...
def outcome_for(experiment_id: str, result: dict[str, Any], result_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "conclusion": (
            f"Frozen annual development evaluation of the pullback-recovery rule: {classification};"
            " no strategy approval, no sealed query, no Champion."
        ),
        "falsified": (
        ...
        "not_falsified": (
            "Other recovery definitions, other means, other barriers, other horizons and all"
            " prospective behaviour remain untested by this declaration."
        ),
        "evidence_facts": [
        ...
        "legitimate_revisit": (
            "Retain this evidence. No exact replay, no mean-period or ratio drift, no added gate"
            " chosen after seeing these numbers; require a new diagnostic or structural prediction"
            " and an explicit cumulative Research Director allocation."
        ),
    }
...
def main() -> None:
    ...
    lab = PullbackLab(inputs, protocol)
    ...
    print("Both bounded WP-006 variants finalized; every negative result is retained.", flush=True)

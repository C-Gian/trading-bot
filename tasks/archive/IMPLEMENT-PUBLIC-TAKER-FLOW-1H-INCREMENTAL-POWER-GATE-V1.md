...
## Owner command (implementation ready)

Prerequisite: the pinned official source under `data/raw/public-taker-flow/` (re-verified
against `data/manifests/BTCUSDT-PUBLIC-TAKER-FLOW-KLINES-DEV-v1.json`; re-acquire with
`.venv\Scripts\python.exe scripts\acquire_public_taker_flow.py` if absent).

Power gate (runs once, refuses to overwrite; `--check` replays and compares by canonical text):
`.venv\Scripts\python.exe scripts\run_public_taker_flow_1h_incremental_power_gate.py`
...
Implementation: `backend/app/predictive/taker_flow_power_gate.py`; governed by
`backend/app/predictive/taker_flow_validation.py` and `scripts/check.py`; tests
`backend/tests/test_public_taker_flow_power_gate.py`.

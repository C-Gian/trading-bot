...
export type ProspectiveObserver={status:'ACTIVE'|'STOPPED'|'DEGRADED';label:string;observer_version:string;evidence_version:string;evidence_stage:string;initiation_mode:string;strategy_version:string;feature_version?:string;research_status?:string;champion_status?:string;max_decision_latency_seconds?:number;first_scientific_review_completed_trades?:number;last_evaluated_hourly_boundary:string|null;last_heartbeat:string|null;next_expected_boundary:string|null;last_successful_market_fetch?:string|null;current_error?:string|null;missed_prospective_decisions:number;raw_prospective_long_signals:number;suppressed_long_signals:number;open_shadow_trade:ShadowTrade|null;completed_shadow_trades:number;manual_evidence_included:boolean;order_placement:boolean;credentials:boolean;real_money:boolean};
